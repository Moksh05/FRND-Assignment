...
const TruckCard = ({ truck }) => {
  const truckInfo = truck.truck; // nested truck object

  const truckLoad = truck.assigned.reduce((sum, l) => sum + l, 0);
  const utilization =
    truckInfo.capacity > 0
      ? ((truckLoad / truckInfo.capacity) * 100).toFixed(2)
      : 0;
    ...
    <div className="border border-gray-200 rounded-xl p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white shadow-sm hover:shadow-md transition-all duration-300">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-sm text-gray-500 flex flex-col sm:flex-row sm:gap-3 mt-1">
          ...
          <p
            className={`text-xs mt-1 ${
              truck.fullyUtilized ? "text-green-600" : "text-orange-500"
            }`}
          >
            {truck.fullyUtilized ? "Fully Utilized" : "Partially Utilized"}
          </p>
        </div>
      </div>
